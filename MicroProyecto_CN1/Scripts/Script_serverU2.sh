...
sleep 10
sudo lxc config device add servidor2 server280 proxy listen=tcp:192.168.100.5:80 connect=tcp:127.0.0.1:80
...
sudo systemctl stop apache2
sleep 10
sudo lxc config device add Bservidor2 server280 proxy listen=tcp:192.168.100.5:1080 connect=tcp:127.0.0.1:80
