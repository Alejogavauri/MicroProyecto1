...
sleep 10
sudo lxc config device add servidor1 server80 proxy listen=tcp:192.168.100.4:80 connect=tcp:127.0.0.1:80
...
sudo systemctl stop apache2
sleep 10
sudo lxc config device add Bservidor1 server80 proxy listen=tcp:192.168.100.4:1080 connect=tcp:127.0.0.1:80
